...
export function ProductDetailClient({ product, reviews, rating }: ProductDetailClientProps) {
  ...
  }, [product, selectedSizeId, soldOut]);
  ...
  if (!product) {
    return (
      ...
      >
        ...
        <p className="mt-2 max-w-xs text-[12px] leading-relaxed text-[color:var(--foreground)]/68">
    ...
    <>
      <div className="relative -mx-3 pb-36">
        ...
        >
          ...
          />
          <motion.div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/55 via-transparent to-[color:rgba(225,189,115,0.15)]" />
          ...
          <div className="pointer-events-none absolute bottom-14 start-3 end-3 flex flex-col items-start gap-2 sm:bottom-16">
            {soldOut ? (
              <span className="rounded-full border border-white/40 bg-black/50 px-3 py-1 text-[11px] font-bold uppercase tracking-wide text-white backdrop-blur-md">
                {t.productPage.soldOutBadge}
              </span>
            ) : null}
        ...
        >
          <div className="text-start">
            ...
            <p className="mt-2 text-[12px] leading-relaxed text-[color:var(--foreground)]/75">
              {product.description[language]}
            </p>
            {soldOut ? (
              <p className="mt-2 rounded-xl border border-[color:var(--border-soft)] bg-[color:var(--card-beige)] px-3 py-2 text-[11px] font-medium text-[color:var(--brand-burgundy-soft)]">
                {t.productPage.soldOutMessage}
              </p>
            ) : null}
            ...
            {product.sizes.map((size) => {
              ...
              return (
                <motion.button
                  ...
                  disabled={soldOut}
                  whileTap={soldOut ? undefined : tapScale}
      ...
      >
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <p className="text-[10px] font-semibold uppercase tracking-wide text-[color:var(--brand-gold-muted)]">
              {t.productPage.total}
            </p>
          ...
          {soldOut ? (
            <span className="inline-flex min-h-12 min-w-[10rem] flex-1 items-center justify-center rounded-2xl border border-[color:var(--border-soft)] bg-[color:var(--card-beige)] px-3 text-center text-[11px] font-semibold text-[color:var(--muted-text)]">
              {t.productPage.soldOutMessage}
            </span>
          ) : (
            <MotionLink
